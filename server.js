...
async function sendCPMRequest(url, payload, headers, params = {}, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`📡 请求 ${attempt}/${maxRetries}: ${fullUrl}`);
...
app.post('/api/get-account-log', async (req, res) => {
  console.log('📜 收到获取账号日志请求');
  
  try {
    const { authToken, email, password } = req.body;
    
    console.log('请求参数:', {
      令牌: authToken ? '已提供' : '未提供',
      邮箱: email ? email.substring(0, 5) + '...' : '未提供'
    });
    ...
    if (!finalAuthToken && email && password) {
      ...
      try {
        ...
        if (loginResponse?.idToken) {
          ...
          console.error('❌ 登录失败:', loginResponse?.error?.message);
          return res.json({
            ok: false,
            error: 401,
            message: `登录失败: ${loginResponse?.error?.message || '未知错误'}`,
            debug: loginResponse
          });
        }
      } catch (loginError) {
        console.error('❌ 登录过程错误:', loginError);
        return res.json({
          ok: false,
          error: 500,
          message: `登录错误: ${loginError.message}`,
    ...
    try {
      ...
      console.log(`🔑 使用Token: ${finalAuthToken?.substring(0, 30)}...`);
      ...
      for (let i = 0; i < payloads.length; i++) {
        ...
        console.log(`📦 尝试payload ${i + 1}/${payloads.length}:`, JSON.stringify(payload));
        
        try {
          const response = await sendCPMRequest(cpmUrl, payload, cpmHeaders, {}, 2);
          ...
          if (response) {
            ...
            if (response.result !== undefined) {
              ...
              console.log(`✅ 从CPM接口获取到数据`);
              cpmSuccess = true;
              break;
            } else if (response.logs) {
              const logs = Array.isArray(response.logs) ? response.logs : [response.logs];
              logs.forEach(log => {
                allLogs.push({
                  source: "cpm_getaccountlog",
                  type: "account_log",
    ...
    try {
      ...
      const playerResponse = await sendCPMRequest(playerUrl, { data: null }, playerHeaders);
      ...
      if (playerResponse?.result) {
        ...
        console.log('📊 玩家数据已添加到日志');
      } else {
        console.log('❌ 获取玩家数据失败');
      }
    } catch (error) {
      console.log('❌ 获取玩家数据失败:', error.message);
    }
    
    // 方法3: 获取车辆数据作为补充
    console.log('🔄 方法3: 获取车辆数据...');
    try {
      ...
      const carsResponse = await sendCPMRequest(carsUrl, { data: null }, carsHeaders);
      ...
      if (carsResponse?.result) {
        ...
        let carsData;
        try {
          carsData = typeof carsResponse.result === 'string' ? 
            JSON.parse(carsResponse.result) : carsResponse.result;
        } catch (e) {
        ...
        const carsLog = {
          ...
          data: {
            total_cars: carCount,
            car_list: Array.isArray(carsData) ? carsData.slice(0, 5).map(car => ({
        ...
        allLogs.push(carsLog);
        console.log(`📊 车辆数据已添加到日志 (${carCount}辆车)`);
      } else {
        console.log('❌ 获取车辆数据失败');
      }
    } catch (error) {
      console.log('❌ 获取车辆数据失败:', error.message);
    }
    
    // 方法4: 尝试其他可能的日志接口
    console.log('🔄 方法4: 尝试其他日志接口...');
    const otherLogEndpoints = [
      "GetPlayerLogs",
      "GetActivityLog",
      "GetUserLogs",
      "GetLoginHistory"
    ...
    for (const endpoint of otherLogEndpoints) {
      try {
        const otherUrl = `${CPM_BASE_URL}/${endpoint}`;
        const otherHeaders = {
          "User-Agent": "okhttp/3.12.13",
          "Authorization": `Bearer ${finalAuthToken}`,
          "Content-Type": "application/json"
        ...
        const response = await sendCPMRequest(otherUrl, { data: null }, otherHeaders, {}, 1);
        
        if (response) {
          console.log(`✅ ${endpoint} 有响应`);
          allLogs.push({
            source: `other_endpoint_${endpoint.toLowerCase()}`,
            type: "system_log",
            timestamp: new Date().toISOString(),
            data: response
          });
    ...
    if (allLogs.length === 0) {
      ...
      allLogs.push(
        ...
        },
        {
          ...
          timestamp: new Date().toISOString(),
          data: {
            message: "无法从服务器获取详细日志",
            suggestion: "请检查账号权限或联系管理员",
            timestamp: new Date().toISOString(),
            api_status: "GetAccountLog接口可能未启用"
          }
        }
      );
    }
    ...
    res.json({ 
      ...
      metadata: {
        ...
        sources_used: [...new Set(allLogs.map(log => log.source))],
        timestamp: new Date().toISOString(),
        auth_status: finalAuthToken ? "authenticated" : "unauthenticated"
      }
    });
...
// 8. 获取Firebase账号信息
app.post('/api/get-firebase-account', async (req, res) => {
  try {
    const { authToken, email, password } = req.body;
    
    console.log('🔐 获取Firebase账号信息请求');
    ...
    if (!finalAuthToken && email && password) {
      ...
      const loginResponse = await sendCPMRequest(loginUrl, loginPayload, loginHeaders, loginParams);
    ...
    if (!finalAuthToken) {
      return res.json({ ok: false, error: 400, message: "缺少认证信息" });
    }
    
    // 获取Firebase账号信息
    const firebaseUrl = "https://www.googleapis.com/identitytoolkit/v3/relyingparty/getAccountInfo";
    const firebasePayload = {
      idToken: finalAuthToken
    };
    ...
    const firebaseParams = { key: FIREBASE_API_KEY };
    
    const firebaseResponse = await sendCPMRequest(firebaseUrl, firebasePayload, firebaseHeaders, firebaseParams);
    
    if (firebaseResponse?.users && firebaseResponse.users.length > 0) {
      const userInfo = firebaseResponse.users[0];
      ...
      const formattedResponse = {
        ...
        data: {
          ...
          lastLoginAt: userInfo.lastLoginAt ? new Date(parseInt(userInfo.lastLoginAt)).toISOString() : null,
          lastRefreshAt: userInfo.lastRefreshAt ? new Date(parseInt(userInfo.lastRefreshAt)).toISOString() : null,
          createdAt: userInfo.createdAt ? new Date(parseInt(userInfo.createdAt)).toISOString() : null,
          validSince: userInfo.validSince ? new Date(parseInt(userInfo.validSince)).toISOString() : null,
          disabled: userInfo.disabled || false,
          localId: userInfo.localId,
          displayName: userInfo.displayName || "未设置",
          photoUrl: userInfo.photoUrl || null,
          providers: userInfo.providerUserInfo || [],
      ...
      res.json({ ok: false, error: 404, message: "未找到账号信息" });
    }
    
  } catch (error) {
    console.error('💥 获取Firebase账号信息错误:', error);
    res.json({ ok: false, error: 500, message: `服务器错误: ${error.message}` });
  }
});
...
app.listen(PORT, () => {
    ...
    ⚡ 版本: 2.5-full-fixed (完整修复版)
    
    可用API端点:
    ====================================
    🔐  POST /api/login              - 账号登录
    📊  POST /api/get-account-data   - 获取账号数据
    🚗  POST /api/get-all-cars       - 获取所有车辆
    🆔  POST /api/change-localid     - 修改账号ID
    🧬  POST /api/clone-account      - 克隆账号
    💰  POST /api/modify-money       - 修改绿钞金币
    📜  POST /api/get-account-log    - 获取账号日志
    🔐  POST /api/get-firebase-account - 获取Firebase账号信息
    🏥  GET  /api/health            - 健康检查
    🧪  GET  /api/test              - 测试端点
    ====================================
