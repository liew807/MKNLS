...
app.post('/api/login', async (req, res) => {
    try {
        ...
        console.log('密钥:', key || '无');
        console.log('Firebase API Key 存在:', !!process.env.FIREBASE_API_KEY);
        ...
        console.log('Firebase URL:', firebaseUrl.substring(0, 100) + '...');
        ...
        if (!firebaseResponse.ok) {
            ...
            await addOperationLog('user_login_failed', email, key || 'N/A', `登录失败: ${errorMsg}`);
        ...
        await addOperationLog('user_login_success', email, key || 'N/A', '用户登录成功');
        ...
        await addOperationLog('user_login_error', 'unknown', 'N/A', `登录接口错误: ${error.message}`);
