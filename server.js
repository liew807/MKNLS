...
app.post('/api/check-key', async (req, res) => {
    try {
        ...
        console.log('🔑 检查秘钥请求:', { key, email: email || '无邮箱' });
        ...
        if (keyData.status !== 'active') {
            console.log('❌ 秘钥状态无效:', keyData.status);
            await addOperationLog('key_verification', email || 'unknown', key, '验证失败: 秘钥状态无效');
            return res.status(400).json({
                success: false,
                message: "秘钥未激活"
            });
        ...
        // 检查是否过期
        const now = new Date();
        const expiryTime = new Date(keyData.expiry_time);
        if (expiryTime < now) {
            console.log('❌ 秘钥已过期:', { now, expiryTime });
    ...
    try {
        ...
        if (key) {
            ...
            console.log('✅ 秘钥验证通过:', {
                ...
                isTestCard: keyData.is_test_card,
                boundEmails: keyData.bound_emails || [],
                maxBind: keyData.max_bind
            });
        }

        // 验证Firebase账号
        console.log('🔥 验证Firebase账号...');
        ...
        if (!firebaseResponse.ok) {
            console.log('❌ Firebase登录失败:', firebaseData.error?.message);
        ...
        if (key && keyData) {
            ...
            if (!boundEmails.includes(email)) {
                console.log('📝 邮箱未绑定，开始绑定...');
                
                // 检查绑定限制（只有普通秘钥才需要检查）
                if (!keyData.is_admin) {
                    const maxBind = keyData.max_bind || 3;
                    if (boundEmails.length >= maxBind) {
                        console.log('❌ 绑定数量已达上限');
                        return res.status(400).json({
                            success: false,
                            message: "该秘钥绑定数量已达上限"
                ...
                try {
                    ...
                    console.error('❌ 更新秘钥绑定失败:', updateError);
                    // 继续登录，但不阻止用户
                }
            } else {
                console.log('✅ 邮箱已绑定，更新最后登录时间');
                // 更新最后登录时间
                const boundAccounts = keyData.bound_accounts || [];
...
app.post('/api/debug', async (req, res) => {
    try {
        const { action, key, email } = req.body;
        
        console.log('\n🔍 调试请求:', { action, key, email });
            ...
            case 'check_key':
                ...
                return res.json({
                    ...
                    data: keyData ? {
                        ...
                        maxBind: keyData.max_bind,
                        boundEmails: keyData.bound_emails,
                        isExpired: new Date(keyData.expiry_time) < new Date()
                    } : { exists: false }
                });
            ...
            default:
                return res.json({
                    ...
                    env: {
                        ...
                        hasRankUrl: !!process.env.RANK_URL,
...
app.listen(PORT, async () => {
    ...
    console.log(`   - RANK URL: ${process.env.RANK_URL ? '✅' : '❌'}`);
