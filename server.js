...
function formatKeyData(keyData) {
    return {
        ...
        // Telegram相关字段
        telegramUser: keyData.telegramUser || (keyData.addedBy === 'telegram_bot' ? {
            id: keyData.telegramUserId,
            username: keyData.telegramUsername,
            firstName: keyData.telegramFirstName
        } : null),
        applicationInfo: keyData.applicationInfo || null,
        // 绑定账号详情
        ...
        isSuperAdmin: keyData.isSuperAdmin,
        activationTime: keyData.activationTime,
        addedBy: keyData.addedBy
    };
}
...
app.get('/api/admin/keys', async (req, res) => {
    try {
        ...
        const tgKeys = allKeys
            .filter(k => k.addedBy === 'telegram_bot')
            .map(formatKeyData);
        
        // 超级管理人生成的秘钥 - 修复：正确筛选超级管理员生成的秘钥
        const superAdminKeys = allKeys
            .filter(k => k.isSuperAdmin && k.addedBy === SUPER_ADMIN_KEY)
            .map(formatKeyData);
        
        // 普通管理人生成的秘钥（按管理员分组）
        const normalAdminKeys = allKeys
            .filter(k => k.isAdmin && !k.isSuperAdmin && k.addedBy !== 'telegram_bot')
            .map(formatKeyData);
...
app.post('/api/admin/keys', async (req, res) => {
    try {
        ...
        const keyData = {
            ...
            status: 'active',
            isAdmin: false, // 生成的秘钥不是管理员秘钥
            isSuperAdmin: false, // 生成的秘钥不是超级管理员秘钥
...
// 10. 查看账号详情 - 对应前端的 viewAccountDetails
app.get('/api/admin/key-details/:key', async (req, res) => {
    try {
        const { key: adminKey } = req.query;
        const { key: targetKey } = req.params;

        if (!adminKey || !targetKey) {
            return res.status(400).json({
                success: false,
                message: "验证失败"
            });
        ...
        if (adminKey !== SUPER_ADMIN_KEY) {
            const keyData = accessKeys.get(adminKey);
            if (!keyData || !keyData.isAdmin) {
                return res.status(403).json({
                    success: false,
                    message: "验证失败"
        ...
        const keyData = accessKeys.get(targetKey);
        if (!keyData) {
