...
io.on('connection', (socket) => {
    ...
    socket.on('chat_message', (data) => {
        ...
        const msg = { id: messages.length + 1, fromId: data.fromId, fromName: data.fromName, fromRole: data.fromRole, toId: data.toId || null, content: data.content, time: new Date().toLocaleString() };
        messages.push(msg);
...
app.post('/api/register', (req, res) => {
    ...
    const newUser = { id: generateUID(), username, password, balance: 0, avatar: '', role, penalty: 0, totalOrders: 0, todayOrders: 0, playerInfo: role === 'player' ? { game: '', price: 0, desc: '', joined: false } : null, vip: null, createdAt: new Date().toLocaleString() };
    users.push(newUser);
    writeData('users.json', users);
...
app.put('/api/user/:id', (req, res) => {
    ...
    if (username && users.find(u => u.username === username && u.id !== parseInt(req.params.id))) return res.json({ code: 400, msg: '用户名已被使用' });
...
app.post('/api/orders', (req, res) => {
    ...
    user.balance -= price; user.totalOrders = (user.totalOrders || 0) + 1; user.todayOrders = (user.todayOrders || 0) + 1;
    writeData('users.json', users);
    const orders = readData('orders.json');
    const newOrder = { id: orders.length ? Math.max(...orders.map(o => o.id)) + 1 : 1, userId, goodsId, goodsName, price, server, gameId, playerId, remark, payMethod: payMethod || '未知', status: 'waiting', createTime: new Date().toLocaleString(), acceptTime: null, completeTime: null };
...
app.put('/api/orders/:id/accept', (req, res) => {
    ...
    order.status = 'accepted'; order.playerId = req.body.playerId; order.acceptTime = new Date().toLocaleString();
    writeData('orders.json', orders);
...
app.put('/api/orders/:id/complete', (req, res) => {
    const orders = readData('orders.json');
    const order = orders.find(o => o.id === parseInt(req.params.id));
    if (!order || order.status !== 'accepted') return res.json({ code: 400, msg: '异常' });
    order.status = 'completed'; order.completeTime = new Date().toLocaleString();
    writeData('orders.json', orders);
    const users = readData('users.json');
    const player = users.find(u => u.id === order.playerId);
    if (player) { player.balance += order.price; player.todayOrders = (player.todayOrders || 0) + 1; player.totalOrders = (player.totalOrders || 0) + 1; writeData('users.json', users); }
...
app.post('/api/join', (req, res) => {
    ...
    const users = readData('users.json');
    const user = users.find(u => u.id === userId);
    if (!user || user.balance < 50) return res.json({ code: 400, msg: '余额不足' });
    user.balance -= 50; user.role = 'player'; user.playerInfo = { game, price: parseFloat(price), desc, joined: true };
    writeData('users.json', users);
...
app.post('/api/penalty/pay', (req, res) => {
    ...
    const amount = user.penalty; user.balance -= amount; user.penalty = 0;
    writeData('users.json', users);
    const finances = readData('finances.json');
...
app.post('/api/invites', (req, res) => {
    const invites = readData('invites.json');
    const code = generateInviteCode(req.body.type);
...
app.get('/api/messages/:type', (req, res) => {
    ...
    res.json({ code: 200, data: messages.filter(m => m.type === req.params.type).reverse() });
});
app.post('/api/messages', (req, res) => {
    const messages = readData('messages.json');
    messages.push({ id: messages.length ? Math.max(...messages.map(m => m.id)) + 1 : 1, from: '官方公告', content: req.body.content, type: 'official', read: false, time: new Date().toLocaleString() });
...
app.put('/api/lottery/price', (req, res) => { const l = readData('lottery.json'); l.price = parseFloat(req.body.price) || 10; writeData('lottery.json', l); res.json({ code: 200 }); });
app.post('/api/lottery/items', (req, res) => { const l = readData('lottery.json'); l.items.push({ name: req.body.name, color: req.body.color || '#ff8c00', prob: parseInt(req.body.prob) || 10 }); writeData('lottery.json', l); res.json({ code: 200 }); });
app.delete('/api/lottery/items/:index', (req, res) => { const l = readData('lottery.json'); l.items.splice(parseInt(req.params.index), 1); writeData('lottery.json', l); res.json({ code: 200 }); });
app.post('/api/lottery/spin', (req, res) => {
    ...
    user.balance -= lottery.price; writeData('users.json', users);
