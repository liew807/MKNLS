...
app.post('/api/login', async (req, res) => {
    ...
    if (!email || !password) {
        return res.json({ ok: false, error: 400, message: "请输入邮箱和密码" });
    }
    ...
    try {
        ...
        if (response.data?.idToken) {
            ...
            res.json({ ok: false, error: 401, message: "登录失败" });
        }
    } catch (error) {
...
app.post('/api/get-account-data', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('📁 已保存到 debug_response.json');
        } catch(e) {
            console.log('⚠️ 无法保存文件:', e.message);
        }
        
        console.log('📦 响应类型:', typeof response);
        if (response) {
            console.log('📦 所有字段:', Object.keys(response));
            console.log('📦 有result:', !!response.result);
            console.log('📦 有data:', !!response.data);
        }
        ...
        if (!response) {
            ...
            try { 
                ...
                // 保存解析后的数据
                fs.writeFileSync('debug_parsed.json', JSON.stringify(data, null, 2));
                console.log('📁 解析后数据保存到 debug_parsed.json');
            } catch(e) { 
                data = response.result;
                console.log('⚠️ result不是JSON');
            }
        ...
        console.log('📋 最终data:', data ? '有数据(' + Object.keys(data).length + '个字段)' : 'null');
        if (data) {
...
app.post('/api/change-localid', async (req, res) => {
    ...
    try {
        ...
        if (saveRes?.result) {
            res.json({ ok: true, result: 1, message: "ID changed!", details: { carsUpdated: 0 } });
        } else {
            res.json({ ok: false, result: 0, message: "Save failed" });
        }
...
app.post('/api/clone-account', async (req, res) => {
    ...
    if (!sourceAuth || !targetEmail || !targetPassword) {
        return res.json({ ok: false, error: 400, message: "Missing parameters" });
    }
    ...
    try {
        ...
        if (!accountResponse?.result) {
            return res.json({ ok: false, error: 404, message: "Failed to get source data" });
        }
        ...
        if (saveRes?.result) {
            res.json({ ok: true, error: 0, message: "Clone successful!", details: { carsCloned: 0, newLocalId: newId } });
        } else {
            res.json({ ok: false, error: 500, message: "Save failed" });
        }
...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', version: '11.0-debug' });
});
