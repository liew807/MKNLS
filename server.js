...
app.post('/api/verify-key', async (req, res) => {
    try {
        ...
        if (!keyData) {
            addLog('key_verification', 'unknown', licenseKey, '验证失败');
            return res.status(400).json({
                success: false,
                message: "验证失败"
            });
...
app.post('/api/verify-admin-key', async (req, res) => {
    try {
        ...
        if (adminKey !== process.env.ADMIN_LOGIN_KEY) {
            addLog('admin_login', 'unknown', adminKey, '验证失败');
            return res.status(400).json({
                success: false,
                message: "验证失败"
            });
        ...
        addLog('admin_login', 'unknown', 'unknown', '验证失败');
        res.status(400).json({
            success: false,
            message: "验证失败"
        });
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!firebaseResponse.ok) {
            addLog('user_login', email, licenseKey || 'N/A', '验证失败');
        ...
        addLog('user_login', 'unknown', 'unknown', '验证失败');
        res.status(400).json({
            success: false,
            message: "验证失败"
        });
