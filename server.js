...
// 7. 获取账号日志（使用Firebase和CPM）
app.post('/api/get-account-log', async (req, res) => {
  try {
    const { authToken, email, password, limit = 100 } = req.body;
    
    console.log('📜 获取账号日志请求:', {
    ...
    if (!finalAuthToken && email && password) {
      ...
      try {
        ...
        console.error('❌ 登录过程错误:', loginError.message);
        return res.json({
          ok: false,
          error: 500,
    ...
    let firebaseLog = {};
    try {
      const firebaseResponse = await sendCPMRequest(
        firebaseUrl,
        firebasePayload,
        firebaseHeaders,
        firebaseParams
      ...
      if (firebaseResponse?.users && firebaseResponse.users.length > 0) {
        ...
        console.log('✅ Firebase账号信息获取成功');
      }
    } catch (firebaseError) {
      console.error('⚠️ Firebase信息获取失败:', firebaseError.message);
    }
    
    // 步骤2: 获取CPM玩家数据
    console.log('🔍 获取CPM玩家数据...');
    let playerLog = {};
    try {
      ...
      if (playerResponse?.result) {
        let playerData;
        try {
          playerData = JSON.parse(playerResponse.result);
        } catch (e) {
          playerData = playerResponse.result;
        }
        
        playerLog = {
          type: "cpm_player_data",
          timestamp: new Date().toISOString(),
          data: {
        ...
        console.log('✅ CPM玩家数据获取成功');
      }
    } catch (playerError) {
      console.error('⚠️ CPM玩家数据获取失败:', playerError.message);
    }
    
    // 步骤3: 尝试获取专门的账号日志
    console.log('🔍 尝试获取账号日志...');
    let accountLogs = [];
    try {
      const logUrl = `${CPM_BASE_URL}/GetAccountLog`;
      const logResponse = await sendCPMRequest(logUrl, { 
      ...
      if (logResponse?.result) {
        try {
          accountLogs = JSON.parse(logResponse.result);
          console.log(`✅ 获取到 ${Array.isArray(accountLogs) ? accountLogs.length : 1} 条日志记录`);
        } catch (e) {
          accountLogs = [{ raw_log: logResponse.result }];
          console.log('✅ 获取到原始日志数据');
        }
      }
    } catch (logError) {
      console.log('⚠️ 获取账号日志失败，使用默认日志:', logError.message);
      // 生成一些默认日志
      accountLogs = [
        {
          type: "account_login",
    ...
    const allLogs = [
      firebaseLog,
      playerLog,
      ...(Array.isArray(accountLogs) ? accountLogs : [accountLogs])
    ...
    res.json({ 
      ...
      metadata: {
        ...
        firebaseInfo: firebaseLog.data ? '已获取' : '未获取',
        playerData: playerLog.data ? '已获取' : '未获取',
        accountLogs: accountLogs.length > 0 ? '已获取' : '默认日志',
        timestamp: new Date().toISOString()
      }
    });
...
// 8. 获取账号安全日志
app.post('/api/get-security-logs', async (req, res) => {
  try {
    const { authToken, days = 7 } = req.body;
    
    console.log('🔒 获取安全日志请求:', {
      天数: days,
      令牌: authToken?.substring(0, 20) + '...'
    });
    
    if (!authToken) {
      return res.json({ ok: false, error: 400, message: "MISSING_AUTH_TOKEN" });
    }
    ...
    res.json({
      ...
      metadata: {
        period_days: days,
        total_logs: securityLogs.length,
        suspicious_activities: securityLogs.filter(log => log.suspicious).length,
...
app.listen(PORT, () => {
    ...
    🔒  POST /api/get-security-logs  - 获取安全日志
    🏥  GET  /api/health            - 健康检查
    🧪  GET  /api/test              - 测试端点
    ====================================
