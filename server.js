...
app.post('/api/login', async (req, res) => {
    try {
        ...
        console.log('正在验证Firebase账号...');
        
        // 验证Firebase账号 - 使用正确的API端点
        ...
        console.log('Firebase响应数据:', JSON.stringify(firebaseData));
        ...
        if (!firebaseResponse.ok) {
            ...
            return res.status(400).json({
                success: false,
                message: errorMsg,
                errorCode: firebaseData.error?.code || 'unknown'
            });
        }
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            superAdminKey: 'cpmMKNLS (已安全存储在服务器端)'
        });
    } catch (error) {
...
app.get('/api/test-firebase', async (req, res) => {
    try {
        ...
        // 即使密码错误，只要API密钥有效就能连接
        res.json({
            success: true,
            firebaseApiKey: process.env.FIREBASE_API_KEY ? '已设置' : '未设置',
            firebaseKeyLength: process.env.FIREBASE_API_KEY ? process.env.FIREBASE_API_KEY.length : 0,
            canConnectToFirebase: response.status !== 400 || data.error?.message?.includes('INVALID_PASSWORD') || data.error?.message?.includes('EMAIL_NOT_FOUND'),
...
app.listen(PORT, async () => {
    ...
    console.log('╠══════════════════════════════════════════════════╣');
    console.log('║ 🔑 超级管理员密钥: cpmMKNLS (只存在于后端)       ║');
    ...
    try {
        ...
        console.log('║ 🔧 Firebase测试: http://localhost:' + PORT + '/api/test-firebase ║');
        console.log('╚══════════════════════════════════════════════════╝');
        console.log('\n🎯 服务已就绪，等待请求...\n');
    } catch (error) {
