...
app.post('/api/unlock-w16-engine', async (req, res) => {
    try {
        ...
        // 解锁W16 8.0引擎相关字段（适配您的游戏）
        parsedData.W1680 = true;
        parsedData.W1680Engine = true;
        parsedData.w1680 = true;
        parsedData.w1680_engine = true;
        parsedData.W16Engine = true;
        parsedData.W16 = true;
        parsedData.w16engine = true;
        parsedData.engineW16 = true;
        parsedData.engineW1680 = true;
        parsedData["W16 8.0"] = true;
        parsedData.unlockedW16 = true;
        parsedData.unlockedW1680 = true;
        ...
        if (saveResult?.result === '{"result":1}' || saveResult?.result === 1) {
            res.json({ ok: true, error: 0, message: "W16 8.0引擎已解锁!" });
        } else {
...
app.post('/api/unlock-premium-houses', async (req, res) => {
    try {
        ...
        parsedData.zhujia3 = true;
        parsedData.zhujia_3 = true;
        parsedData.zhuJia3 = true;
        parsedData.premiumHouse = true;
...
app.get('/api/health', (req, res) => {
    res.json({ 
        ...
        endpoints: {
            total: 14,
            features: ['登录', '获取数据', '修改ID', '克隆账号', '修改货币', '解锁W16 8.0引擎', '解锁住家3', '解锁烟雾', '修改名字无限制', '解锁无限油', '解锁无伤模式', '修改胜场', '一键全解锁']
        }
    });
});
