...
app.post('/api/unlock-w16-engine', async (req, res) => {
    try {
        ...
        parsedData.hasW16 = true;
        parsedData.w16Unlocked = true;
...
app.post('/api/unlock-smokes', async (req, res) => {
    try {
        ...
        parsedData.smoke11 = true;
        parsedData.smoke12 = true;
        parsedData.smoke13 = true;
        parsedData.smoke14 = true;
        parsedData.smoke15 = true;
...
app.post('/api/change-name-unlimited', async (req, res) => {
    try {
        ...
        if (!authToken || !newName) {
            return res.json({ ok: false, error: 400, message: "缺少认证令牌或新名字" });
        }
...
app.post('/api/unlock-god-mode', async (req, res) => {
    try {
        ...
        parsedData.currentHealth = 999999;
        parsedData.damageImmune = true;
...
app.post('/api/modify-wins', async (req, res) => {
    try {
        ...
        if (!authToken) {
            return res.json({ ok: false, error: 400, message: "缺少认证令牌" });
        }
        ...
        if (operationType === 'set') {
            ...
            if (isNaN(winsValue)) {
                return res.json({ ok: false, error: 400, message: "胜场数无效" });
            }
            newWins = winsValue;
        } else if (operationType === 'add') {
            const winsValue = Number(wins);
...
app.get('/api/health', (req, res) => {
    res.json({ 
        ...
        endpoints: {
            总数: 14,
            功能: ['登录', '获取数据', '修改ID', '克隆账号', '修改货币', '解锁W16引擎', '解锁付费房屋', '解锁烟雾', '修改名字无限制', '解锁无限油', '解锁无伤模式', '修改胜场', '一键全解锁']
        }
    });
});
...
app.get('/api/test', (req, res) => {
    res.json({
        status: 'ok',
        message: 'CPM游戏助手API正常运行',
        timestamp: new Date().toISOString(),
        环境: process.env.NODE_ENV || '开发环境',
        firebase密钥: FIREBASE_API_KEY ? '已设置' : '未设置'
    });
});
...
// 404处理
app.use((req, res) => {
    res.status(404).json({ error: '页面未找到', 路径: req.path });
});
...
app.listen(PORT, () => {
    ...
    🔑 Firebase API密钥: ${FIREBASE_API_KEY ? '已设置 ✓' : '未设置 ✗'}
    ⚡ 版本: 3.0-完整版 (增强功能版)
