...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!email || !password) {
            ...
            await addOperationLog('user_login', email || 'unknown', key || 'N/A', '登录失败: 邮箱或密码为空');
            return res.status(400).json({
                success: false,
                message: "请提供邮箱和密码"
            });
        ...
        if (!emailRegex.test(email)) {
            console.error('❌ 登录失败：邮箱格式无效');
            await addOperationLog('user_login', email, key || 'N/A', '登录失败: 邮箱格式无效');
            return res.status(400).json({
                success: false,
                message: "邮箱格式无效"
            });
        ...
        if (key) {
            ...
            if (!keyData) {
                ...
                await addOperationLog('user_login', email, key, '登录失败: 无效的秘钥');
                return res.status(400).json({
                    success: false,
                    message: "无效的秘钥"
                });
            ...
            if (keyData.status !== 'active') {
                ...
                await addOperationLog('user_login', email, key, '登录失败: 秘钥未激活');
                return res.status(400).json({
                    success: false,
                    message: "秘钥未激活"
                });
            ...
            if (new Date(keyData.expiry_time) < new Date()) {
                ...
                await addOperationLog('user_login', email, key, '登录失败: 秘钥已过期');
                return res.status(400).json({
                    success: false,
                    message: "秘钥已过期"
                });
            ...
            if (!keyData.is_admin) {
                ...
                if (!isEmailBound && boundEmails.length >= maxBind) {
                    ...
                    await addOperationLog('user_login', email, key, '登录失败: 绑定已达上限');
                    return res.status(400).json({
                        success: false,
                        message: "该秘钥绑定数量已达上限"
                    });
...
app.listen(PORT, async () => {
    ...
    await initDatabase();
    console.log(`🔑 超级管理员密钥: ${SUPER_ADMIN_KEY} (只存在于后端)`);
