...
app.post('/api/unlock-w16-engine', async (req, res) => {
    try {
        ...
        // 2. 诊断现有数据
        console.log('🔍 诊断当前引擎字段:');
        const engineFields = {};
        Object.keys(parsedData).forEach(key => {
            if (key.toLowerCase().includes('engine') || 
                key.toLowerCase().includes('w16') || 
                key.toLowerCase().includes('w1680')) {
                engineFields[key] = parsedData[key];
                console.log(`  ${key}: ${parsedData[key]}`);
            }
        });
        
        // 3. 尝试所有可能的W16字段
        console.log('🔧 设置W16 8.0引擎字段...');
        ...
        w16Fields.forEach(field => {
            parsedData[field] = 1; // 使用1而不是true，有些游戏可能用数字
        });
        ...
        // 方法3: 尝试中文字段
        parsedData['引擎'] = 'W16 8.0';
        parsedData['当前引擎'] = 'W16 8.0';
            ...
            if (carsData?.result) {
                try {
                    ...
                    if (Array.isArray(cars) && cars.length > 0) {
                        console.log(`找到 ${cars.length} 辆车，更新前3辆...`);
                        ...
                        for (let i = 0; i < Math.min(3, cars.length); i++) {
                            ...
                            // 更新车辆引擎
                            carCopy.engine = 'W16 8.0';
                            carCopy.Engine = 'W16 8.0';
                            carCopy.engineType = 'W16 8.0';
                            carCopy.engineLevel = 5;
                            carCopy.enginePower = 1200;
                            ...
                            console.log(`✅ 车辆 ${i + 1} 引擎已更新`);
                            await new Promise(resolve => setTimeout(resolve, 300));
                        }
                    }
                } catch (e) {
                    console.log('解析车辆数据失败，跳过车辆更新:', e.message);
                }
            }
        } catch (carsError) {
            console.log('获取车辆数据失败，跳过:', carsError.message);
        }
        ...
        if (saveResult?.result === '{"result":1}' || saveResult?.result === 1 || saveResult?.result === "1") {
            console.log('✅ W16 8.0引擎解锁成功');
            
            // 返回详细诊断信息
            res.json({ 
                ok: true, 
                error: 0, 
                message: "W16 8.0引擎解锁尝试完成！",
                details: {
                    originalFields: engineFields,
                    fieldsSet: w16Fields.length + 10, // 估算设置的字段数
                    carsUpdated: Math.min(3, (carsData?.result ? JSON.parse(carsData.result).length : 0)),
                    note: "请重启游戏查看效果。如果仍失败，请提供更多游戏信息以便调试。"
                }
            });
        } else {
            ...
            res.json({ 
                ok: false, 
                error: 500, 
                message: "保存数据失败，但已尝试设置所有可能字段",
                debug: {
                    saveResult: saveResult,
                    fieldsSet: w16Fields,
                    note: "API返回保存失败，但数据可能已部分更新"
                }
            });
        }
    } catch (error) {
        console.error('💥 解锁W16 8.0引擎错误:', error);
        res.json({ 
            ok: false, 
            error: 500, 
            message: `SERVER_ERROR: ${error.message}`,
            suggestion: "请提供游戏截图或更多信息，以便调整解锁方法"
        });
    }
});
...
app.post('/api/unlock-premium-houses', async (req, res) => {
    try {
        ...
        if (saveResult?.result === '{"result":1}' || saveResult?.result === 1) {
            res.json({ ok: true, error: 0, message: "住家3已解锁!" });
        } else {
...
app.post('/api/modify-wins', async (req, res) => {
    try {
        ...
        const currentWins = parsedData.wins || parsedData.Wins || 0;
