    ...
    try {
        ...
        await pool.query(`
            CREATE TABLE IF NOT EXISTS access_keys (
                ...
                is_password_card BOOLEAN DEFAULT FALSE,  -- 新增：改密卡
                ...
                card_type VARCHAR(50) DEFAULT 'FULL',     -- FULL/TEST/PASSWORD
                ...
                is_telegram_generated BOOLEAN DEFAULT FALSE,  -- 新增：TG机器人生成标志
                kuaishou_code VARCHAR(100) DEFAULT NULL,     -- 新增：快手暗号
                applied_via VARCHAR(50) DEFAULT NULL         -- 新增：申请渠道
            )
        `);
...
function formatKeyData(keyData) {
    return {
        ...
        isPasswordCard: keyData.is_password_card || false,  // 新增
...
app.get('/api/admin/keys', async (req, res) => {
    try {
        ...
        // Telegram机器人生成的秘钥
        const tgKeys = formattedKeys.filter(k => k.isTelegramGenerated || k.addedBy === 'telegram_bot');
...
app.post('/api/admin/copy-key', async (req, res) => {
    try {
        ...
        const keyData = {
            ...
            isPasswordCard: originalKeyData.is_password_card || false,
            ...
            addedByName: adminInfo.name,
            addedByEmail: adminInfo.email,
            cardType: originalKeyData.card_type || 'FULL',
            activationTime: now.toISOString(),
            originalKey: keyToCopy,
            copiedTimes: 0,
            isTelegramGenerated: originalKeyData.is_telegram_generated || false
        };
...
// 14. 新增：获取测试卡和改密卡统计信息
app.get('/api/admin/card-stats', async (req, res) => {
    try {
        const { key } = req.query;
        ...
        const statsQuery = `
            SELECT 
                COUNT(*) as total,
                SUM(CASE WHEN is_test_card = true THEN 1 ELSE 0 END) as test_cards,
                SUM(CASE WHEN is_password_card = true THEN 1 ELSE 0 END) as password_cards,
                SUM(CASE WHEN is_test_card = false AND is_password_card = false THEN 1 ELSE 0 END) as full_cards,
                SUM(CASE WHEN status = 'active' AND expiry_time > NOW() THEN 1 ELSE 0 END) as active_cards,
                SUM(CASE WHEN status = 'expired' OR expiry_time <= NOW() THEN 1 ELSE 0 END) as expired_cards
            FROM access_keys
        `;
...
// 15. 新增：通过Telegram机器人添加秘钥（适配前端TG分类）
app.post('/api/admin/add-telegram-key', async (req, res) => {
    try {
        const { key } = req.query;
        const { 
            telegramUser, 
        ...
        // 确定卡类型
        const isTestCard = cardType === 'TEST';
        const isPasswordCard = cardType === 'PASSWORD';
        
        // 创建秘钥数据
        const keyData = {
            key: newKey,
            remark: `TG申请: ${telegramUser.firstName || '用户'} - ${kuaishouCode || '无暗号'}`,
            expiryTime: expiryTime.toISOString(),
            status: 'active',
            isAdmin: false,
            isSuperAdmin: false,
            ...
            maxBind: isTestCard || isPasswordCard ? 1 : 3,
            boundEmails: [],
            boundAccounts: [],
            addedBy: 'telegram_bot',
            addedByName: telegramUser.firstName || 'TG用户',
            addedByEmail: `tg_${telegramUser.id}@telegram.org`,
            cardType: cardType,
            activationTime: now.toISOString(),
            originalKey: null,
            copiedTimes: 0,
            isTelegramGenerated: true,
            telegramUser: telegramUser,
            kuaishouCode: kuaishouCode,
            appliedVia: appliedVia || 'telegram_bot'
        };
        
        const createdKey = await createAccessKey(keyData);
        
        await addOperationLog('tg_generate_key', 'telegram_bot', newKey, 
               `通过TG生成${isTestCard ? '测试卡' : isPasswordCard ? '改密卡' : cardType + '卡'}`);
        
        res.json({
            success: true,
            key: newKey,
            message: "Telegram秘钥生成成功",
            cardInfo: {
                type: cardType,
        ...
        console.error('TG生成秘钥错误:', error);
        await addOperationLog('tg_generate_key', 'telegram_bot', 'unknown', 'TG生成秘钥失败');
        res.status(400).json({
            success: false,
            message: "验证失败"
        });
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        res.json({ 
            status: 'ok', 
            message: 'Backend is running with Enhanced Admin Management',
            database: 'connected',
...
app.listen(PORT, async () => {
    ...
    console.log('   • 增强的用户权限验证');
    console.log('   • Telegram机器人生成接口');
