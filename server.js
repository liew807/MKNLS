...
app.post('/api/login', async (req, res) => {
    try {
        ...
        console.log('登录请求:', { 
            ...
            key: key ? key.substring(0, 8) + '...' : '未提供秘钥'
        });
        ...
        console.log('Firebase响应:', {
            success: firebaseResponse.ok,
            status: firebaseResponse.status,
            error: firebaseData.error || '无错误'
        });

        if (!firebaseResponse.ok) {
            // 翻译Firebase错误消息
            const firebaseError = firebaseData.error;
            let errorMessage = '登录失败';
            
            if (firebaseError && firebaseError.message) {
                if (firebaseError.message.includes('INVALID_EMAIL')) {
                    errorMessage = "邮箱地址无效";
                } else if (firebaseError.message.includes('INVALID_PASSWORD')) {
                    errorMessage = "密码错误";
                } else if (firebaseError.message.includes('USER_DISABLED')) {
                    errorMessage = "账号已被禁用";
                } else if (firebaseError.message.includes('USER_NOT_FOUND')) {
                    errorMessage = "用户不存在";
                } else if (firebaseError.message.includes('TOO_MANY_ATTEMPTS_TRY_LATER')) {
                    errorMessage = "尝试次数过多，请稍后再试";
                } else if (firebaseError.message.includes('EMAIL_NOT_FOUND')) {
                    errorMessage = "邮箱未注册";
                } else {
                    errorMessage = `登录失败: ${firebaseError.message}`;
                }
            }
            
            await addOperationLog('user_login', email, key || 'N/A', `登录失败: ${errorMessage}`);
        ...
        console.log('Firebase登录成功:', { 
            ...
            userId: firebaseData.localId.substring(0, 8) + '...' 
        });
        ...
        if (key) {
            try {
                ...
                console.error('绑定邮箱失败:', bindError);
                // 继续执行，不阻止登录
            }
        }
        ...
        await addOperationLog('user_login', 'unknown', 'N/A', `登录接口错误: ${error.message}`);
        res.status(400).json({
            success: false,
