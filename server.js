...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.send(`
      ...
      <html>
      <head>
        <title>Car Parking Multiplayer Tool</title>
        <style>
          body { font-family: Arial, sans-serif; padding: 40px; text-align: center; }
          .container { max-width: 600px; margin: 0 auto; }
          h1 { color: #333; }
          .status { background: #4CAF50; color: white; padding: 15px; border-radius: 5px; margin: 20px 0; }
          .endpoints { text-align: left; background: #f5f5f5; padding: 20px; border-radius: 5px; }
          code { background: #e0e0e0; padding: 2px 5px; border-radius: 3px; }
        </style>
      </head>
      <body>
        <div class="container">
          <h1>🚗 Car Parking Multiplayer Tool</h1>
          <div class="status">
            ✅ 后端服务器运行正常
          </div>
          <p>服务器端口: <code>${PORT}</code></p>
          <div class="endpoints">
            <h3>📋 可用API端点：</h3>
            <ul>
              <li><code>POST /api/login</code> - 登录账号</li>
              <li><code>POST /api/account-info</code> - 获取账号信息</li>
              <li><code>POST /api/modify-gold</code> - 修改金币</li>
              <li><code>POST /api/modify-money</code> - 修改绿钞</li>
              <li><code>POST /api/modify-localid</code> - 修改LocalID</li>
              <li><code>POST /api/clone-account</code> - 克隆账号</li>
              <li><code>POST /api/test</code> - 测试API</li>
              <li><code>GET /health</code> - 健康检查</li>
            </ul>
          </div>
          <p style="margin-top: 30px; color: #666;">
            请将前端HTML文件放入 <code>public</code> 目录中
          </p>
        </div>
      </body>
      </html>
