class cpmcylone {
    ...
    initFeaturesSection() {
        ...
        if (selectAllBtn) {
            selectAllBtn.addEventListener('click', () => this.toggleSelectAllFeatures());
        }
        
        // 应用功能按钮
        const applyFeaturesBtn = document.getElementById('apply-features-btn');
        if (applyFeaturesBtn) {
            applyFeaturesBtn.addEventListener('click', () => this.applyFeatures());
        }
        
        // 添加Enter键支持
        const raceCountInput = document.getElementById('race-count');
        const playerNameInput = document.getElementById('player-name');
        
        if (raceCountInput) {
            raceCountInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
    ...
    toggleSelectAllFeatures() {
        ...
        if (!featureCheckboxes.length) return;
        ...
        if (selectAllBtn) {
            if (allChecked) {
                selectAllBtn.innerHTML = '<i class="fas fa-check-double"></i> 全选所有功能';
            } else {
    ...
    // 新增：应用高级功能 - 修复版，使用正确的API端点
    async applyFeatures() {
        if (this.isProcessing) {
            console.log('正在处理中，请稍候...');
            this.showStatus('error', '请等待，另一个操作正在进行中', 'features-status');
            return;
        }

        if (!this.sourceAuth) {
            this.showStatus('error', '请先登录账号', 'features-status');
            this.addLog('✗ 未找到认证令牌');
            return;
        }
        ...
        // 获取其他输入值
        const raceCount = document.getElementById('race-count')?.value.trim();
        const playerName = document.getElementById('player-name')?.value.trim();

        // 验证输入
        if (selectedFeatures.length === 0 && !raceCount && !playerName) {
            this.showStatus('error', '请至少选择一个功能或输入修改内容', 'features-status');
            return;
        }

        // 验证比赛场数
        if (raceCount && (isNaN(raceCount) || raceCount < 0 || raceCount > 9999)) {
            this.showStatus('error', '比赛场数必须是0-9999之间的数字', 'features-status');
            return;
        }

        // 验证玩家名字
        if (playerName && playerName.length > 20) {
            this.showStatus('error', '玩家名字不能超过20个字符', 'features-status');
            return;
        }

        // 确认操作
        let confirmMessage = '⚠️ 确认应用以下高级功能？\n\n';
        
        if (selectedFeatures.length > 0) {
            confirmMessage += '解锁功能:\n';
            selectedFeatures.forEach(feature => {
                const featureName = this.getFeatureName(feature);
                confirmMessage += `• ${featureName}\n`;
            });
        }
        ...
        if (playerName) {
            confirmMessage += `玩家名字: 修改为 "${playerName}"\n`;
        }

        if (!confirm(confirmMessage)) {
            this.addLog('✗ 用户取消功能应用操作');
            return;
        }
        ...
        this.updateButtonState('apply-features-btn', true, '应用中...');
        this.clearStatusLog();
        this.updateProgress('开始应用高级功能...', 5);
        this.updateTimeEstimate();
        this.addLog('开始应用高级功能...');

        // 显示选择的功能
        if (selectedFeatures.length > 0) {
            const featureNames = selectedFeatures.map(f => this.getFeatureName(f)).join(', ');
            this.addLog(`已选择功能: ${featureNames}`);
        }
        if (raceCount) {
            this.addLog(`比赛场数: ${raceCount}`);
        }
        if (playerName) {
            this.addLog(`玩家名字: ${playerName}`);
        }
        ...
        try {
            ...
            for (const feature of selectedFeatures) {
                const success = await this.applySingleFeature(feature);
                if (!success) {
                    allSuccess = false;
                    errorMessages.push(this.getFeatureName(feature));
                }
            }
            
            // 处理比赛场数
            if (raceCount) {
                const success = await this.modifyWins(raceCount, 'set');
                if (!success) {
                    allSuccess = false;
                    errorMessages.push('比赛场数');
                }
            }
            
            // 处理玩家名字
            if (playerName) {
                const success = await this.changePlayerName(playerName);
                if (!success) {
                    allSuccess = false;
                    errorMessages.push('玩家名字');
                }
            }
            ...
            if (allSuccess) {
                this.updateProgress('所有功能应用完成！', 100);
                this.addLog('✓ 所有高级功能应用成功！');
                
                // 显示成功详情
                if (selectedFeatures.length > 0) {
                    selectedFeatures.forEach(feature => {
                        const featureName = this.getFeatureName(feature);
                        this.addLog(`✓ ${featureName}: 已解锁`);
                    });
                }
                
                if (raceCount) {
                    this.addLog(`✓ 比赛场数: 已设置为 ${raceCount}`);
                }
                
                if (playerName) {
                    this.addLog(`✓ 玩家名字: 已修改为 "${playerName}"`);
                }
                
                this.addLog(`总耗时: ${elapsedTime} 秒`);
                this.showStatus('success', `高级功能应用成功！耗时 ${elapsedTime} 秒`, 'features-status');
                this.updateTimeEstimate('已完成');
                
                this.showSuccessAnimation();
                
                // 清空输入框
                if (document.getElementById('race-count')) {
                    document.getElementById('race-count').value = '';
                }
                ...
                this.addLog(`✗ 部分功能应用失败: ${errorMessages.join(', ')}`);
                this.showStatus('warning', `部分功能应用失败: ${errorMessages.join(', ')}`, 'features-status');
                this.updateProgress('部分完成', 70);
                this.updateTimeEstimate('部分完成');
            }

        } catch (error) {
            console.error('功能应用错误:', error);
            
            this.addLog(`✗ 错误: ${error.message}`);
            this.showStatus('error', `功能应用失败: ${error.message}`, 'features-status');
            
            this.updateProgress('应用中断', 0);
            this.updateTimeEstimate('已中断');
            this.showErrorAnimation();
        } finally {
            this.isProcessing = false;
            this.updateButtonState('apply-features-btn', false, '应用所选功能');
        }
    }

    // 新增：应用单个功能
    async applySingleFeature(featureCode) {
        try {
            const apiUrl = this.getFeatureApiUrl(featureCode);
            if (!apiUrl) {
                console.error(`未找到功能 ${featureCode} 的API地址`);
                return false;
            }
            
            this.addLog(`正在应用: ${this.getFeatureName(featureCode)}...`);
            ...
            if (data.ok || data.error === 0) {
                this.addLog(`✓ ${this.getFeatureName(featureCode)}: 成功`);
                return true;
            } else {
                console.error(`功能 ${featureCode} 失败:`, data.message);
                this.addLog(`✗ ${this.getFeatureName(featureCode)}: ${data.message || '失败'}`);
                return false;
            }
            
        } catch (error) {
            console.error(`功能 ${featureCode} 请求错误:`, error);
            this.addLog(`✗ ${this.getFeatureName(featureCode)}: ${error.message}`);
            return false;
        }
    }

    // 新增：修改胜场数
    async modifyWins(winsCount, operationType = 'set') {
        try {
            this.addLog(`正在修改比赛场数为 ${winsCount}...`);
            ...
            if (data.ok || data.error === 0) {
                this.addLog(`✓ 比赛场数: 已修改为 ${winsCount}`);
                return true;
            } else {
                console.error('修改比赛场数失败:', data.message);
                this.addLog(`✗ 比赛场数: ${data.message || '修改失败'}`);
                return false;
            }
            
        } catch (error) {
            console.error('修改比赛场数请求错误:', error);
            this.addLog(`✗ 比赛场数: ${error.message}`);
            return false;
        }
    }
    ...
    async changePlayerName(newName) {
        try {
            ...
            if (data.ok || data.error === 0) {
                this.addLog(`✓ 玩家名字: 已修改为 "${newName}"`);
                return true;
            } else {
                console.error('修改玩家名字失败:', data.message);
                this.addLog(`✗ 玩家名字: ${data.message || '修改失败'}`);
                return false;
            }
            
        } catch (error) {
            console.error('修改玩家名字请求错误:', error);
            this.addLog(`✗ 玩家名字: ${error.message}`);
            return false;
        }
    }

    // 新增：获取功能对应的API地址
    getFeatureApiUrl(featureCode) {
        const apiMap = {
            'w16': '/api/unlock-w16-engine',
            'houses': '/api/unlock-premium-houses',
            'smoke': '/api/unlock-smokes',
            'infinite-gas': '/api/unlock-unlimited-fuel',
            'no-damage': '/api/unlock-god-mode'
        };
        
        return apiMap[featureCode] ? `${this.baseUrl}${apiMap[featureCode]}` : null;
    }
    ...
    checkSession() {
        ...
        if (savedAuth) {
            ...
            this.showElement('features-section');
    ...
    logout() {
        ...
        this.hideElement('features-section');
    ...
    updateButtonState(buttonId, disabled, text) {
        ...
        if (disabled) {
            ...
            if (buttonId === 'clone-btn') {
                ...
                icon = 'fa-coins';
            } else if (buttonId === 'apply-features-btn') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.log('应用版本: 2.5 (修复高级功能API调用)');
