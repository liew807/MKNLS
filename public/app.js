class cpmcylone {
    ...
    async refreshMoney() {
        ...
        try {
            ...
            if (data.ok && data.data) {
                ...
                try {
                    accountData = JSON.parse(data.result);
                } catch(e) {
            ...
            if (accountData) {
                ...
                const goldCoins = accountData.coin || accountData.Coin || accountData.money || 0;
                ...
                // Token可能过期
                console.log('获取数据失败，可能token过期:', data);
                this.logout();
                this.showStatus('error', '会话已过期，请重新登录', 'money-status');
            }
        } catch (error) {
            console.error('刷新货币信息失败:', error);
            this.showStatus('error', '刷新失败，请检查网络', 'money-status');
        }
    }

    // ==================== 【修复2】modifyMoney - 使用独立接口 ====================
    async modifyMoney() {
        ...
        if (operationType === 'max') {
            ...
            // 验证输入值
            if (greenCashValue && (isNaN(greenCashValue) || Number(greenCashValue) < 0)) {
                this.showStatus('error', '绿钞值必须是非负数字', 'money-status');
                return;
            }
            
            if (goldCoinsValue && (isNaN(goldCoinsValue) || Number(goldCoinsValue) < 0)) {
        ...
        let confirmMessage = '';
        if (operationType === 'max') {
            confirmMessage = '⚠️ 确认将绿钞和金币都设置为最大值 999,999,999？';
        } else {
            confirmMessage = `⚠️ 确认修改货币？\n\n操作类型: ${operationType === 'set' ? '设置为' : '增加'}\n`;
            if (greenCashValue) confirmMessage += `绿钞: ${this.formatNumber(greenCashValue)}\n`;
            if (goldCoinsValue) confirmMessage += `金币: ${this.formatNumber(goldCoinsValue)}`;
        }
        ...
        try {
            ...
            // 一键最大
            if (operationType === 'max') {
                this.updateProgress('正在设置最大值...', 20);
                ...
                if (data.ok) {
                    ...
                    this.showStatus('success', `✅ 货币已设置为最大值！耗时 ${elapsedTime} 秒`, 'money-status');
                } else {
                    // 检查是否token过期
                ...
                // 修改绿钞
                if (greenCashValue) {
                    this.updateProgress('正在修改绿钞...', 30);
                    this.addLog(`💚 修改绿钞: ${operationType === 'set' ? '设置为' : '增加'} ${this.formatNumber(greenCashValue)}`);
                ...
                // 修改金币
                if (goldCoinsValue) {
                    this.updateProgress('正在修改金币...', 60);
                    this.addLog(`💛 修改金币: ${operationType === 'set' ? '设置为' : '增加'} ${this.formatNumber(goldCoinsValue)}`);
                ...
                allSuccess = greenSuccess && goldSuccess;
            }
    ...
    // ==================== 【修复3】verifyAndLoadAccount - 修复会话过期 ====================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.log('应用版本: 3.0 (修复会话过期+独立货币接口)');
    } catch (error) {
