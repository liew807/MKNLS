class cpmcylone {
    ...
    async verifyAndLoadAccount(authToken) {
        try {
            ...
            if (data.data) {
                ...
                console.log('获取失败，完整响应:', data);
                this.addLog('✗ 获取数据失败: ' + (data.message || '请重试'));
                // 不退出登录，让用户重试
            }
        } catch (error) {
            console.log('网络错误:', error);
            this.addLog('✗ 网络错误，请重试');
        }
    }
    ...
    displayAccountInfo(accountData) {
        ...
        document.getElementById('account-name').textContent = accountData.Name || accountData.username || '未知';
        document.getElementById('account-money').textContent = this.formatNumber(accountData.coin || accountData.Coin || accountData.money || 0);
        document.getElementById('account-green-cash').textContent = this.formatNumber(accountData.cash || accountData.Cash || 0);
        document.getElementById('account-localid').textContent = accountData.localID || accountData.localId || '未知';
        
        const badge = document.getElementById('account-status');
        if (badge) { badge.textContent = '已登录'; badge.setAttribute('data-status', 'online'); }
    }
    ...
    logout() {
        ...
        const badge = document.getElementById('account-status');
        if (badge) { badge.textContent = '未登录'; badge.setAttribute('data-status', 'offline'); }
